...
* EMACS CONFIG
:PROPERTIES:
:header-args: elisp :tangle init.el :comments no
:END:
...
:PROPERTIES:
:header-args: elisp :tangle early-init.el :comments no
:END:
...
#+begin_src elisp
  (if (eq (getenv "WORK-EMACS") "Y")
      (load-file "~/.config/emacs/work-init.el")
...
*** CONFIG. WSL2
:PROPERTIES:
:header-args: elisp :tangle wsl-init.el :comments no
:END:
...
*** CONFIG. WINDOWS
:PROPERTIES:
:header-args: elisp :tangle win-init.el :comments no
:END:

**** FUENTES (Windows)

#+begin_src elisp
  (set-face-attribute 'default nil :family "CaskaydiaMono NFP" :height 120)
#+end_src
