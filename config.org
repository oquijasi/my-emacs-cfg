...
*** Desactivar Org built-in

Desactivamos la build de Org que viene en Emacs para cargar la que le indiquemos en la config y asegurar que usamos siempre la misma.
...
#+begin_src elisp
  (when (eq system-type 'android) (progn (setenv "PATH" (format "%s:%s" "/data/data/com.termux/files/usr/bin" (getenv "PATH"))) (push "/data/data/com.termux/files/usr/bin" exec-path)))
#+end_src
...
#+RESULTS:
: [nil 26643 39162 820297 nil elpaca-process-queues nil nil 941000 nil]
